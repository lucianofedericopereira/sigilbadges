...
(() => {
  ...
  const STORAGE_KEY = 'sigilbadges-locale';
  ...
  function ensureGoogleTranslateLoaded() {
    ...
    googleReadyPromise = new Promise((resolve) => {
      const includedLanguages = LOCALES.filter((l) => l.code !== 'en')
        .map((l) => l.code)
        .join(',');
      window.__sigilbadgesInitGoogleTranslate = () => {
      ...
      loadScript('https://translate.google.com/translate_a/element.js?cb=__sigilbadgesInitGoogleTranslate');
    });
